...
from .anthropic import (
    anthropic_request_to_messages, build_message_response,
    stream_text_events, stream_full_response_events,
)
...
class GeminiHandler(BaseHTTPRequestHandler):
    ...
    def do_HEAD(self):
        # Some clients (e.g. Claude Code) send HEAD probes against arbitrary
        # paths (like /api/hello) to check connectivity. BaseHTTPRequestHandler
        # has no default do_HEAD, which causes a 501. Just respond 200/404.
        try:
    ...
    def _handle_anthropic_messages(self, body: bytes):
        ...
        messages, tools, tool_choice = anthropic_request_to_messages(req)
        ...
        if stream and (not tools or tool_choice in ("none", None)):
            try:
                self._start_sse()
